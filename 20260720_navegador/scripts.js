...
function mostrarMensajes(){
  ...
  for(let i = 0; i < mensajes.length; i++){
    ...
    <p>${mensaje}</p>
    <button class='btnEliminar' data-indice='${i}'>Eliminar mensaje</button>
    </article>
    `
  }
...
function eliminarMensaje(e){
  ...
  let confirmar = confirm('No sé yo... ¿Estás seguro?')
  if(confirmar === true){
    let confirmar2 = confirm('última oportunidad')
    if(confirmar === true){
    mensajes.splice(indice, 1)  //índice y cuántos elementos quieres eliminar
    mostrarMensajes()
    }
  }
