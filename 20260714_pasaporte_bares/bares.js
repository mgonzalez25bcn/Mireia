...
function sellarBar(idBar){
  ...
  if (bar.visitado === true){
    return 'Este bar ya estaba sellado' //ojo
  }
...
function crearMensajeProgreso(visitados, total){
  ...
  const mensaje = `Has visitado ${visitados} bares, de un total de ${total} bares. Tu
  progreso es de un ${porcentaje}%`
  return mensaje
}
